...
pub fn solve(input1: String, _: String, _: &[String]) {
    ...
    let raw_module_fuel: i32 = module_weights
        .iter()
        .map(calc_fuel_for)
        .sum();
    ...
    println!("Rocket requires {} units of fuel for modules", raw_module_fuel);
    println!("Rocket requires {} units of fuel in total", total_module_fuel);
}
